...
function createRow(user){
    ...
    trElm.innerHTML = `<tr>
                ...
                <td>
                    ${!user.verified ? '<button type="button" onclick="approveUser(event)" id="approveBtn" class="btn btn-success" data-user-id="' + user.id + '" >&check;</button>' : ""}
                    <button type="button" id="delete" onclick="deleteUser(event)"   class="btn btn-danger" data-user-id="` + `${user.id}` + `">X</button>
...
function approveUser(e) {
    if (confirm("Are you sure")) {
        ...
        fetch(`${domain}/users/${e.target.getAttribute('data-user-id')}`, {
            ...
            .then((res) => res.json())
            .then((data) => {
                e.target.parentElement.parentElement.children[4].innerText = 'Accepted';
                e.target.remove();
            });
    }
}
...
document.addEventListener('DOMContentLoaded', function (){
    ...
    fetch(`${domain}/users`)
        ...
        .then((data) => {
            data.forEach((user) => {
                document.getElementById('table_id').children[1].appendChild(createRow(user));
            });
            $('#table_id').DataTable();
        });
})